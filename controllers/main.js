...
const mainCtrl={
    getPosts:async(req,res)=>{
        try {
          ...
          const gh=allPosts.filter(item=>item.post.includes('مانتو')).slice(0,4)
          const ds=allPosts.filter(item=>item.post.includes('بلوز')).slice(0,4)
          const gho=allPosts.filter(item=>item.post.includes('شلوار')).slice(0,4)
